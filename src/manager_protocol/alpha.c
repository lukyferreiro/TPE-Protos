...
int alpha_req_to_packet(char* output, struct alpha_req* input, int* size) {
    ...
    int aux;
    *size = get_packet_size(ALPHA_REQUEST, input->command, input->data.string);
    ...
    alpha_data_to_buffer(input->data, cmd_to_req_data_type(input->command), buffer_p);
...
int alpha_res_to_packet(char* output, struct alpha_res* input, int* size) {
    ...
    int aux;
    *size = get_packet_size(ALPHA_RESPONSE, input->command, input->data.string);
    char* buffer_p = output;
    ...
    if (input->status == SC_OK) {
        alpha_data_to_buffer(input->data, cmd_to_res_data_type(input->command), buffer_p);
    }
